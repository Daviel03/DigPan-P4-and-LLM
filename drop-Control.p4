...
parser MyParser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_meta) {
    state start {
...
control MyIngress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_meta) {
    ...
    table ipv4_filter {
        ...
        default_action = forward(0);  // Default to port 0, overridden by entries
        size = 1024;
    }

    apply {
        if (hdr.ipv4.isValid()) {
            ipv4_filter.apply();  // Apply filter table, use default if no match
